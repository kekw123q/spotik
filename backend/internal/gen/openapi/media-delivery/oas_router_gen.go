		...
		case '/': // Prefix: "/"
			...
			case 's': // Prefix: "stream/"
				...
				if len(elem) == 0 {
					...
					switch r.Method {
					case "GET":
						s.handleGetStreamUrlRequest([1]string{
							args[0],
						}, elemIsEscaped, w, r)
					default:
		...
		case '/': // Prefix: "/"
			...
			case 's': // Prefix: "stream/"
				...
				if len(elem) == 0 {
					...
					switch method {
					case "GET":
						r.name = GetStreamUrlOperation
						r.summary = "Получить ссылку на поток"
						r.operationID = "getStreamUrl"
						r.pathPattern = "/stream/{resourceId}"
						r.args = args
						r.count = 1
						return r, true
					default:
			...
			case 'u': // Prefix: "upload"
				...
				if len(elem) == 0 {
					...
					case "POST":
						...
						r.summary = "Загрузка медиа-файла"
